...
# dentro do teto de resolução.
#
# Usado também (mesma lógica de $SCALE e dos ramos de encoder) por
# sample-test.sh, que reencoda só um trecho de 60s pra testar/calibrar as
# configs antes de rodar em lote — mudanças aqui devem ser espelhadas lá.
#
# Uso:
#   transcode-1080p-hevc.sh <diretorio|arquivo> [--replace] [--dry-run]
#                           [--encoder auto|cpu|nvenc] [--quality N]
#                           [--resolution N] [--fps N]
#
...
#   --encoder E     "cpu" (libx265), "nvenc" (GPU NVIDIA) ou "auto" (padrão:
#                   detecta VAAPI/Intel Quick Sync, senão cai pra CPU — é o
#                   único modo que usa VAAPI, não dá pra pedir explicitamente)
#   --quality N     qualidade do encoder, padrão 24. Significa CRF no libx265
#                   (cpu) ou CQ no NVENC — em ambos, menor = mais qualidade/
#                   maior arquivo. Ignorado no modo VAAPI (usa qp fixo).
...
#   ./transcode-1080p-hevc.sh /mnt/data/filmes/filme.mkv --encoder nvenc --quality 26 --replace
...
if [[ -z "$DIR" || ( ! -d "$DIR" && ! -f "$DIR" ) ]]; then
  echo "Uso: $0 <diretorio|arquivo> [--replace] [--dry-run] [--encoder auto|cpu|nvenc] [--quality N] [--resolution N] [--fps N]" >&2
  exit 1
fi
...
if [[ "$ENCODER" == "nvenc" ]]; then
  ...
  # auto: detecta VAAPI de verdade (não só se o encoder existe, mas se abre o
  # device), senão cai pra CPU. É o único jeito de acabar usando VAAPI.
  if [[ -e "$VAAPI_DEV" ]]; then
...
case "$MODE" in
  vaapi) echo "Aceleração: VAAPI (Intel Quick Sync) detectada e funcionando." | tee -a "$LOG" ;;
  nvenc) echo "Aceleração: NVENC (GPU NVIDIA), cq=$QUALITY." | tee -a "$LOG" ;;
  cpu)   echo "Aceleração: CPU (libx265), crf=$QUALITY." | tee -a "$LOG" ;;
esac
